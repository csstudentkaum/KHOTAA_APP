...
class _NotificationBellWidgetState extends State<NotificationBellWidget>
  ...
  StreamSubscription? _firestoreUnreadSubscription;
  int _alertUnreadCount = 0;
  int _firestoreUnreadCount = 0;

  int get _unreadCount => _alertUnreadCount + _firestoreUnreadCount;
  ...
  void initState() {
    ...
    AlertService().addListener(_updateAlertUnreadCount);
    _updateAlertUnreadCount();

    // Listen for Firestore notification unread count
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      _firestoreUnreadSubscription = NotificationService()
          .streamUnreadCount(user.uid)
          .listen((count) {
            if (mounted) {
              setState(() {
                _firestoreUnreadCount = count;
              });
            }
          });
    }
  ...
  void _updateAlertUnreadCount() {
    if (mounted) {
      final newCount = AlertService().unreadCount;
      debugPrint('NotificationBell: Updating alert unread count to $newCount');
      setState(() {
        _alertUnreadCount = newCount;
      });
    }
  }
  ...
  void dispose() {
    ...
    _firestoreUnreadSubscription?.cancel();
    AlertService().removeListener(_updateAlertUnreadCount);
    super.dispose();
  }
...
class AppBarNotificationBell extends StatefulWidget {
  ...
  const AppBarNotificationBell({Key? key, this.onTap}) : super(key: key);
