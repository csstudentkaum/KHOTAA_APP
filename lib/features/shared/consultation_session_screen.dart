...
class _ConsultationSessionScreenState extends State<ConsultationSessionScreen> {
  ...
  void initState() {
    ...
    _messagesSubscription = _service
        .streamMessages(widget.consultationId)
        .listen((_) {
  ...
  Future<void> _pickFile() async {
    ...
    setState(() => _isSending = true);
    try {
      await _service.sendFile(widget.consultationId, File(pf.path!), pf.name);
      _scrollToBottom();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _initiateVideoCall(Consultation consultation) async {
    ...
    final myName = me?.displayName ?? (widget.isDoctor ? 'Doctor' : 'Patient');
  ...
  /// Show the rating popup when the consultation transitions to completed/followUp
  /// on the patient side, OR when the patient opens an unrated followUp session.
  void _maybeShowRatingDialog(Consultation consultation) {
    final newStatus = consultation.status;
    ...
    if (_ratingDialogShown || consultation.rated == true) {
      _previousStatus = newStatus;
      return;
    }

    // Trigger on:
    // 1) Status transition to completed or followUp
    // 2) First load into a followUp session that hasn't been rated
    final isTransition =
        _previousStatus != null &&
        _previousStatus != newStatus &&
        (newStatus == 'completed' || newStatus == 'followUp');

    final isUnratedFollowUp =
        _previousStatus == null && newStatus == 'followUp';

    _previousStatus = newStatus;

    if (!isTransition && !isUnratedFollowUp) return;
    _ratingDialogShown = true;

    // Schedule after frame to avoid showing during build
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      RatingDialog.show(
        context,
        doctorId: consultation.doctorID,
        doctorName: consultation.doctorName ?? 'the Doctor',
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<Consultation?>(
      ...
      builder: (context, consultSnap) {
        ...
        if (consultation != null && consultation.isFollowUpExpired) {
        ...
        final isReadOnly =
            consultation != null &&
            (consultation.isCompleted || consultation.isFollowUpExpired);

        // Show rating popup when status transitions to completed/followUp (patient only)
        if (consultation != null) {
          _maybeShowRatingDialog(consultation);
        } else {
          _previousStatus ??= null;
        }
        ...
        return Scaffold(
          ...
          body: Column(
            children: [
              ...
              // ── Rate Doctor banner (patient in follow-up, not yet rated) ──
              if (consultation != null &&
                  !widget.isDoctor &&
                  consultation.status == 'followUp' &&
                  consultation.rated != true &&
                  !_ratingDismissed)
                Container(
                  color: Colors.white,
                  padding: const EdgeInsets.symmetric(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        GestureDetector(
                          onTap: () async {
                            final rated = await RatingDialog.show(
                              context,
                              doctorId: consultation.doctorID,
                              doctorName:
                                  consultation.doctorName ?? 'the Doctor',
                              consultationId: consultation.consultationID,
                              patientId: consultation.patientID,
                            );
                      ...
                      itemBuilder: (context, i) {
                        ...
                        if (msg.type == MessageType.videoCall) {
                          for (int j = i + 1; j < messages.length; j++) {
                            ...
                            if (messages[j].type == MessageType.videoCall)
                              break;
                          }
                        }
                        return _MessageBubble(
                          ...
                          onPlayVoice: (msgId, pathname) async {
                            if (_playingMessageId == msgId) {
                              ...
                              try {
                                final signedUrl = await _service.getFileUrl(
                                  pathname,
                                );
                  ...
                  _CheckInFloatingBar(
                    onTap: () async {
                      final result = await Navigator.of(context).push<bool>(
                        MaterialPageRoute(
                          builder: (_) =>
                              FollowUpCheckInScreen(consultation: consultation),
                        ),
                      );
                      if (result == true && mounted) _scrollToBottom();
                    },
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  isActive ? 'Online' : _statusLabel(consultation?.status),
                  style: const TextStyle(
                    fontSize: 11,
        ...
        return GestureDetector(
          onTap: message.fileUrl != null
              ? () async {
                  ...
                  final name =
                      message.fileName ?? url.split('?').first.split('/').last;
