...
class _OtpScreenState extends State<OtpScreen> {
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      ...
      await _authService.verifyOTP(verificationId: _verificationId, otp: code);

      if (!mounted) return;

      if (_isLogin) {
        // Login flow – migrate user doc if UID changed (common in debug auth)
        await _authService.migrateUserDocIfNeeded();

        // Fetch actual role from Firestore (now at the correct UID key)
        String role = _registrationData['role'] as String? ?? 'patient';
        try {
          final actualRole = await _authService.getCurrentUserRole();
          if (actualRole != null) {
            role = actualRole;
          }
        } catch (e) {
